...
class SliderCreator:
    ...
    def create_slider_endpoints(self, points, time_ms, start_frame, end_frame):
        # Slider-Kopf erstellen
        head_x, head_y = points[0]
        self.create_slider_endpoint(head_x, head_y, f"slider_head_{time_ms}", start_frame)
        ...
        self.create_slider_endpoint(end_x, end_y, f"slider_tail_{time_ms}", end_frame)

    def create_slider_endpoint(self, x, y, name, frame):
        ...
        # Benutzerdefiniertes Attribut "show" hinzufügen
        endpoint["show"] = False
        endpoint.keyframe_insert(data_path='["show"]', frame=frame - 1)

        endpoint["show"] = True
        endpoint.keyframe_insert(data_path='["show"]', frame=frame)
